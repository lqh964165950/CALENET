...
class PEM(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, scale=0.1, map_reduce=8):
        ...
        self.branch0 = nn.Sequential(
            ...
            BasicConv(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=1),
            SeparableConvBNReLU( 2* inter_planes, 2 * inter_planes, kernel_size=7, dilation=1, stride=1)
        )
        self.branch1 = nn.Sequential(
            ...
            SeparableConvBNReLU( 2* inter_planes, 2 * inter_planes, kernel_size=13, dilation=1, stride=1)
        )
        self.branch2 = nn.Sequential(
            ...
            SeparableConvBNReLU( 2* inter_planes, 2 * inter_planes, kernel_size=21, dilation=1,stride=1)
        )
...
class DPFE(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, p=1, g=1, act=True):
        ...
        c_= c2//2
        self.conv1_1 = Conv(c1//2, c_,k=3,s=1,g=1,act=True)
        self.conv1_2 = Conv(c_, c_, k=5, s=1,g=c_, act=True)
        self.conv3_1 = Conv(c1//2, c_,k=3,s=2,g=1,act=True)
        self.conv3_2 = Conv(c_, c_,k=5,s=1,g= c_,act=True)
        deformable_groups = 1
        offset_channels = 18
        mask_channels = 9
    ...
    def forward(self, x):
        x1,x2 = x.chunk(2,1)
        x1 = self.conv1_1(x1)
        x1 = torch.cat((x1,self.conv1_2(x1)),1)
        offset = self.conv2_offset(x1)
        mask = torch.sigmoid(self.conv2_mask(x1))
        x1 = self.act2(self.bn2(self.conv2(x1, offset=offset, mask=mask))) 
        x2 = self.conv3_1(x2)
        x2 = torch.cat((x2,self.conv3_2(x2)),1)
        x = x1 + x2
        return x
